...
снижается вероятность дедлоков.
* С целью снижения вероятности дедлоков в БД в некоторых случаях используюся запросы `SELECT ... FOR UPDATE`.
При возникновении дедлока запрос вернется в очередь и будет обработан снова (благодаря fail-fast).
...
может помочь упростить логику работы с БД, уменьшить количество запросов.
* Изменить жизненный цикл БД транзакций для возможности отката 
в середине транзакции сервиса. Может поможет написать код, снижающий вероятность дедлоков.
* Рефакторить с целью лучшего следования SOLID и повышения тестируемости кода, 
в т.ч. вынести бизнес-логику из моделей в соотвествующие сервисы. Реализовать unit тесты.
* Реализовать нагрузочное тестирование, в т.ч. с оценкой потребляемой памяти.
 
Дополнительно имеет смысл рассмотреть вопрос о предотвращении засорения 
очереди сбойными сообщениями (которые сервис не смог обработать).

Наиболее правильным на мой взгляд решением, с точки зрения реализации RabbitMQ, 
в данном случае является установка ttl по времени для сообщения и использования 
dead letter exchange. Такой способ хорошо сочетается с fail-fast методологией сервиса. 
С другой стороны в таком случае возможна потеря сообщений, если сообщения 
будут не успевать обрабатываться воркерами.

Еще одним из вариантов может быть создание своего расширения для RabbitMQ, 
которое реализует ttl, аналогичный ttl из TCP.